...
import { createUser, createDoveFund, createDoveCampaign, sleep, getBalance, equalDateTime, getNow } from "./util";
...
describe("test_create_dove_fund", () => {
    ...
    it("createDoveFund", async () => {
        ...
        assert.equal(doveCampaignAccount.campaignName, "Test Porject 3");
        ...
        assert.ok(equalDateTime(doveFundAccount0.createdDate.toNumber(), dove_fund1_created_date));
        assert.ok(equalDateTime(doveFundAccount0.updateDate.toNumber(), dove_fund1_created_date));
